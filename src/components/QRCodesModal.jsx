...
export default function QRCodesModal({ restaurant, onClose }) {
  ...
  useEffect(() => {
    async function gen() {
      const url = `${baseUrl}/app/r/${restaurant.slug}`
      try {
  ...
  // Generate table QRs — encode the in-app URL with table query param
  useEffect(() => {
    async function generateQRs() {
      const codes = {}
      for (const table of tables) {
        const url = `${baseUrl}/app/r/${restaurant.slug}?table=${encodeURIComponent(table.table_number)}`
        try {
  ...
  const printRestaurantQR = () => {
    const html = buildQRPrintHTML({
      ...
      type: 'restaurant',
      items: [{ qrDataUrl: restaurantQR, label: restaurant.name, sublabel: 'Scan to view menu & order', url: `${baseUrl}/app/r/${restaurant.slug}` }],
    })
  ...
  const printTableQRs = () => {
    const items = tables.map(t => ({
      ...
      url: `${baseUrl}/app/r/${restaurant.slug}?table=${encodeURIComponent(t.table_number)}`,
    }))
    const html = buildQRPrintHTML({
      restaurantName: restaurant.name,
      type: 'table',
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-black/8 shrink-0">
          <div>
            ...
            <p className="text-sm text-black/45">Generate, download and print QR codes</p>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-black/5 transition-colors"
          >
            <X className="w-5 h-5 text-black/45" />
          </button>
        </div>

        {/* Tabs */}
        <div className="flex border-b border-black/8 px-6 shrink-0">
          <button
            onClick={() => setActiveTab('restaurant')}
            className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'restaurant'
                ? 'border-orange-600 text-black'
                : 'border-transparent text-black/45 hover:text-black/25'
            }`}
          >
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="max-w-md mx-auto text-center">
              <div className="bg-gradient-to-br from-white to-white rounded-2xl border border-black/10 p-8">
                <div className="inline-flex items-center gap-1.5 bg-black text-black px-3 py-1 rounded-full text-xs font-semibold mb-4">
                ...
                <p className="text-xs text-black/55 font-mono mt-2">{baseUrl}/app/r/{restaurant.slug}</p>
            ...
            <div>
              {tables.length === 0 ? (
                <div className="text-center py-16">
                  <QrCode className="w-12 h-12 text-black/80 mx-auto mb-3" />
                  <p className="text-black/45 font-medium">No tables set up</p>
                ...
                <>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-white text-black px-4 py-2 rounded-xl text-sm font-medium hover:bg-[#F4F4F4]"
                    >
                      <Printer className="w-4 h-4" />
                      Print All (4 per A4)
                    </button>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    {tables.map(table => (
                      <div key={table.id} className="bg-gradient-to-br from-white to-white rounded-xl border border-black/8 overflow-hidden">
                        <div className="p-3 flex justify-center">
                          {qrCodes[table.id] ? (
                            <img src={qrCodes[table.id]} alt={`Table ${table.table_number}`} className="w-28 h-28 rounded-lg" />
                          ) : (
                            <div className="w-28 h-28 flex items-center justify-center">
                              <QrCode className="w-10 h-10 text-black/80" />
                            </div>
                          )}
                        </div>
                        <div className="px-3 pb-3 text-center">
                          <div className="inline-flex items-center gap-1 bg-black/10 text-black px-2 py-0.5 rounded-full text-[10px] font-semibold mb-1">
                            DINE IN
                          </div>
                          ...
                          <button
                            ...
                            className="mt-2 flex items-center justify-center gap-1.5 w-full px-3 py-1.5 bg-white border border-black/10 rounded-lg text-xs font-medium text-black/35 hover:bg-black/5"
                          >
